...
logfilepath = basepath + "/TumorStats_old"+str(pop_size)+"_"+str(cores)+"_"+str(replica)+".csv"
resultfile = open(basepath+"TumorRuntimes_old"+str(nnodes)+"_"+str(cores)+"_"+str(replica)+".txt", "a")

db_path = "sqlite:///" + os.path.join(basepath+"/database", "TumorRes"+str(pop_size)+"_"+str(cores)+"_"+str(replica)+".db")

db_path_OLD = "sqlite:///" + os.path.join(basepath+"/database", "TumorRes_old"+str(pop_size)+"_"+str(cores)+"_"+str(replica)+".db")
...
redis_sampler = pyabc.sampler.RedisEvalParallelSampler(host=host, port=port, adapt_look_ahead_proposal=True, look_ahead = True, look_ahead_delay_evaluation=True, log_file=basepath + "/TumorStats_OLD"+str(pop_size)+"_"+str(cores)+"_"+str(replica)+".csv")
starttime=time()
...
abc.new(db_path_OLD, data_mean)
history = abc.run(max_nr_populations=max_nr_pop, minimum_epsilon=min_eps)
endtime=time()

resultfile.write("LA_OLD, " + str(endtime-starttime)+", " + str(pop_size) + ", " + str(cores) + ", " + str(min_eps)+ ", " + str(replica))


df, w = history.get_distribution(m=0, t=history.max_t)
pyabc.visualization.plot_kde_matrix(df, w, limits=limits);
plt.savefig(os.path.join(basepath,"TumorResPPP_OLD"+str(pop_size)+"_"+str(cores)+"_"+str(replica)+".pdf"))
