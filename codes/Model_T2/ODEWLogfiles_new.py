...
for psize in pop_sizes:
        ...
        for i in range(0,iters_PPP):
            ...
            redis_sampler = pyabc.sampler.RedisEvalParallelSampler(host=host,
                                                 port=port,
                                                 look_ahead = True, adapt_look_ahead_proposal=False,
...
# Parameter inference using LA_pre scheduling (iters_PPP_pre repetitions)
...
for psize in pop_sizes:
        ...
        for i in range(0,iters_PPP):
            ...
            logfilepath = os.path.join(basepath, "results/Var"+str(sleepvar)+"/logfiles", "Logs_pre"+str(nodes)+"_"+str(psize)+"_"+str(i)+".csv")


            redis_sampler = pyabc.sampler.RedisEvalParallelSampler(host=host,
                                                 port=port,
                                                 look_ahead = True, adapt_look_ahead_proposal=True,
                                                 look_ahead_delay_evaluation = False,
                                                 log_file=logfilepath)
            ...
            db_path = "sqlite:///" + os.path.join(basepath,
                                                  "results/Var"+str(sleepvar)+"/database",
                                                  "DYNLA_pre"+str(nodes)+"_"+str(psize)+"_"+str(i)+".db")
